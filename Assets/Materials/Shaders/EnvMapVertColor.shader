    ...
    Properties
    {
		...
		Pass{
			...
			vertOutput vert(appdata_full input) {
				...
				//calculating uv
				output.uvRaw = TRANSFORM_TEX(input.texcoord, _MainTex);
				...
				//output.color = float4(ShadeVertexLightsFull(input.vertex, input.normal, 4, true), 1.0);
				//output.color *= input.color;
				output.color = input.color;

				//Fog
				float distance = length(UnityObjectToViewPos(input.vertex));
			...
			float4 frag(vertOutput input) : COLOR
			{
				...
				half4 c2 = tex2D(_MainTex2, input.uvRaw);
				half4 color = c * c2 * (input.colorFog.a);
